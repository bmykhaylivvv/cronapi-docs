...
Log into the [dashboard](https://cronapi.dev/dashboard) and navigate to the **API Key** page. Your API key is generated automatically on first visit. Copy it — you will only see the full key once.
...
```bash
curl -X POST https://cronapi.dev/api/v1/crons/{your_cron_id}/trigger \
  -H "Authorization: Bearer your_api_key_here"
```
...
```bash
curl https://cronapi.dev/api/v1/crons/{your_cron_id}/runs \
  -H "Authorization: Bearer your_api_key_here"
```
